...
class BrandProductClass extends Common
{
    ...
    public function getAllData(){
        $data = $this::where(['status'=>1])->order('sort desc')->select();
        return $data;
    }
    
    //获取所有可用数据
    public function getDataByBrandId($brand_id){
        $data = $this::where(['status'=>1,'brand_id'=>$brand_id])->order('sort desc')->select();
        return $data;
    }
    
    //分页数据
    public function getPageData($where=['status'=>['neq',-1]],$sort='sort desc'){
        $data = $this::field('id,status,sort,title')->where($where)->order($sort)->select();
        return $data;
    }
    ...
    public function saveData($data,$where=[]){
        $saveType = 'BrandProductClass.add';
        
        if(!empty($where)){$saveType = 'BrandProductClass.edit';}
