...
class Rule extends Common
{
    ...
    public function __construct(Request $request = null,$options = [])
    {
        ...
        $this->model = new \app\index\model\AuthRule();
    }
    ...
    //添加权限
    public function add(){
        $parentData = $this->model->getDataByPid(0);//一级菜单
        $this->assign('parent',$parentData);
    ...
    public function getList(){
        ...
        foreach($parentData as $k=>$v){
            $parentData[$k]['childs'] = $this->model->getDataByPid($v['id']);//一级菜单
   ...
   public function del(){
       ...
       $childData = $this->model->getDataByPid($id);//是否还存在下级
