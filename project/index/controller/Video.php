...
class Video extends Common
{
    ...
    public function index()
    {
        ...
        if (isset($get['group_id']) && $get['group_id'] != 0) {
            $where['a.group_id'] = $get['group_id'];
        }
        if (isset($get['keyword'])) {
            $where['a.title'] = ['like', '%' . $get['keyword'] . '%'];
        }
        $countPage = $this->VideoModel->getCountMsg($where);
    ...
    public function getDataByWhere()
    {
        ...
        $data = $this->VideoModel->getPageData($page, $count, $where);
    ...
    public function edit()
    {
        ...
        $data = $this->VideoModel->getDataByWhere(['id' => $id]);
    ...
    public function saveData()
    {
        ...
        if ($data['url'] == "") {
            return ['status' => -1, 'msg' => "必须上传视频"];
        }

        if (empty($data['id'])) {
            //添加数据
            $data['times'] = time();
            $return = $this->VideoModel->saveData($data);
            if ($return) {
            ...
            $id = $data['id'];
            unset($data['id']);
            $return = $this->VideoModel->updateData($data, ['id' => $id]);
    ...
    public function editStatus()
    {
        ...
        $return = $this->VideoModel->updateData($data, ['id' => $get['id']]);
    ...
    public function uploadImg()
    {
        ...
        return uploadImg($file, '/upload/Video/images/');
    }
    ...
    public function uploadVideo()
    {
        ...
        return uploadVideo($file, '/upload/Video/video/');
    }
